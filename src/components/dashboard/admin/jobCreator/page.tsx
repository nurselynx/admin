...
const JobCreator: React.FC<DetailsFacilityProps> = ({
  setShowDetails,
  showDetails,
}) => {
  const renderDetail = (
        ...
        <div className=" px-6 pb-3 w-full">
          {showDetails?.requestedBy && (
            <>
              ...
              {renderDetail(
                "Address",
                decryptData(showDetails?.requestedByAddress ?? "", secretKey)
              )}
              <hr />
              ...
              {renderDetail(
                "Type",
                decryptData(showDetails?.requestedByType ?? "", secretKey) ||
